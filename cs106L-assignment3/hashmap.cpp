...
template<typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap &hashMap):HashMap(hashMap.bucket_count(), hashMap._hash_function) {
...
template<typename K, typename M, typename H>
HashMap<K, M, H> &HashMap<K, M, H>::operator=(const HashMap &hashMap) {
    if (this == &hashMap)
        return *this;
    else {
        clear();
        ...
        _hash_function = hashMap._hash_function;
        for (const auto &value: hashMap) {
            insert(value);
        }
        return *this;
...
template<typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap &&hashMap) noexcept:HashMap(hashMap.bucket_count(), hashMap._hash_function) {
    _size = std::move(hashMap._size);
    _hash_function = std::move(hashMap._hash_function);
    _buckets_array = std::move(hashMap._buckets_array);
}

template<typename K, typename M, typename H>
HashMap<K, M, H> &HashMap<K, M, H>::operator=(HashMap &&hashMap) noexcept {
    if (this == &hashMap)
        return *this;
    else {
        clear();
